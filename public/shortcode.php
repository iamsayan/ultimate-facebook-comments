...
function ufc_load_fb_comments_shortcode() {
    
    $options = get_option( 'ufc_plugin_global_options' );

    if ($options['ufc_fb_comment_loading_method'] == 'On Click') {
        $get_btn = '<button id="ufc-button" class="' . esc_html($options['ufc_loading_button_class']) . '" onclick="showUFC(); return false;">' . esc_html($options['ufc_loading_button_text']) . '</button><br><br>';
    } else {
        $get_btn = '';
    }
    ...
    if ($options['ufc_fb_comment_loading_method'] == 'On Click') {
        $shortcode_content = $cookie_consent . '<div id="' . esc_html($options['ufc_comment_area_id']) . '" class="ufc-comments ' . esc_html($options['ufc_comment_area_class']) . '" style="text-align:center;' . $get_bgcolor . '">' . $get_btn . '</div>';
    } else {
        $shortcode_content = $cookie_consent . '<div id="' . esc_html($options['ufc_comment_area_id']) . '" class="ufc-comments ' . esc_html($options['ufc_comment_area_class']) . '" style="' . $get_bgcolor . '">' . ufc_comments_area_content() . '</div>';
    }
